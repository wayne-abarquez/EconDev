(function(){
    ...
    function gmapServices($log, $q) {
        ...
        service.defaultLatLng = new google.maps.LatLng(34.860585909463552, -90.260770132377928);
        ...
        service.loadFusionTableLayer = loadFusionTableLayer;
        service.hideFusionTableLayer = hideFusionTableLayer;
        service.loadGeoJson = loadGeoJson;
        service.removeGeoJson = removeGeoJson;
        service.initHeatmap = initHeatmap;
        ...
        function loadFusionTableLayer (selectColumn, tableId, _opts) {
            if (!service.apiAvailable()) return;
        ...
        function hideFusionTableLayer (layer) {
            if (!service.apiAvailable()) return;

            if(layer && layer.getMap()) {
                layer.setMap(null);
            }
        }

        service.geojsonInfoWindow = service.createInfoWindow('');

        function loadGeoJson(json, opts) {
            /*
             * Returns Features
             * It can be used to remove geojson
             * on the Map
             */
            var features = service.map.data.addGeoJson(json);

            /* Apply Icon Style Here */
            /* TODO find a way to set icon for separate geojson's */
            // works but for global
            //service.map.data.setStyle({icon: opts.iconStyle});
            /* Inefficient Hack */
            if (opts.icon) {
                features.forEach(function (feature) {
                    service.map.data.overrideStyle(feature, {icon: opts.icon});
                });
            }

            service.map.data.addListener('click', function (event) {
                if( !opts.clickCallback) {
                    ...
                    content += "";
                    service.geojsonInfoWindow.setContent(content);
                } else opts.clickCallback(event);

                var pixelOffset = opts.pixelOffset || new google.maps.Size(0, -20);
                service.geojsonInfoWindow.setOptions({pixelOffset: pixelOffset});
                service.geojsonInfoWindow.setPosition(event.latLng);
                service.geojsonInfoWindow.open(service.map);
            });
        ...
        function removeGeoJson(features) {
            if (!service.apiAvailable()) return;

            if (features.length > 0) {
                features.forEach(function (feature) {
                    service.map.data.remove(feature);
                });
            }
        }
        ...
        function initHeatmap (points, gradient, radius, maxIntensity) {
            if (!service.apiAvailable()) return;
            ...
            var h = new google.maps.visualization.HeatmapLayer({
                ...
                dissipating: false,
                radius: radius || 0.1,
                maxIntensity: maxIntensity || 0.5
            });

            if(gradient) h.set('gradient', gradient);
